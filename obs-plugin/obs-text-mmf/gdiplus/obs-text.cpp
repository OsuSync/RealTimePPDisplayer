...
#define S_FONT                          "font"
#define S_MMF_NAME                      "mmf_name"
#define S_MMF_READ_INTELVAL		"mmf_read_intelval"
...
#define T_MMF_NAME                      T_("MMF.Name")
#define T_MMF_READ_INTELVAL             T_("MMF.Read.Interval")
...
public:
	...
	{
		handle=CreateFileMappingA(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, 1024, map_name.c_str());
		//handle = OpenFileMappingA(FILE_MAP_READ, false, map_name.c_str());
		data = (char*)MapViewOfFile(handle, FILE_MAP_READ, 0, 0, 1024);
	}
...
struct TextSource {
	...
	string map_name;
	unique_ptr<MappingFile> mapping_file;
	float mmf_read_intelval;
...
void TextSource::LoadMappingText()
{
	text = to_wide(mapping_file->ReadToEnd());
...
inline void TextSource::Update(obs_data_t *s)
{
	...
	const char *new_mapping= obs_data_get_string(s, S_MMF_NAME);
	...
	mmf_read_intelval = obs_data_get_double(s,S_MMF_READ_INTELVAL)/1000.0;
	...
	map_name = new_mapping;
	mapping_file = unique_ptr<MappingFile>(new MappingFile(map_name));
	LoadMappingText();
...
inline void TextSource::Tick(float seconds)
{
	...
	if (update_time_elapsed >= mmf_read_intelval) {
...
static obs_properties_t *get_properties(void *data)
{
	...
	obs_properties_add_float_slider(props, S_MMF_READ_INTELVAL, T_MMF_READ_INTELVAL,1000.0f/60.0f,1000, 1000.0f / 60.0f);
	obs_properties_add_text(props, S_MMF_NAME, T_MMF_NAME, OBS_TEXT_DEFAULT);
...
bool obs_module_load(void)
{
	...
	si.get_defaults = [] (obs_data_t *settings)
	{
		...
		obs_data_set_default_double(settings, S_MMF_READ_INTELVAL, 33);
